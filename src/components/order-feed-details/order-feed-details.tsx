...
import { getOrder } from "../../services/order-details/slice";
...
export const OrderFeedDetails = () => {
    ...
    const { profileOrders } = useAppSelector(getProfileOrderFeedPath);

    const order = useAppSelector((store) => {
        let order = totalOrders?.find(order => order.number === Number(number));
        if (order) {
            return order;
        };

        order = profileOrders?.find(order => order.number === Number(number));
        if (order) {
            return order;
        };
    ...
    const counter = (ingredientId: string) => {
        let counter = 0;
